...
class Neurons(BaseModel):
    ...
    def create_vector(self, doc, user_openai_api_key=None):
        ...
        if user_openai_api_key and not local_embed:
            self.commons["documents_vector_store"]._embedding = OpenAIEmbeddings(
                openai_api_key=user_openai_api_key
            )
            print("Using OPENAI EMBEDDINGS!!!!")  # pyright: ignore reportPrivateUsage=none
        else:
            self.commons["documents_vector_store"]._embedding = local_embeddings
